...
const DATASET  = 'sunti';
const TABLE    = 'payment_schedule_files';
...
exports.payment_schedule_files = async (req, res) => {
  setCors(res);
  ...
  try {
    if (req.method === 'POST' && path === '/signed-upload-url') {
      const { schedule_id, file_name, content_type } = req.body || {};
      if (!schedule_id || !file_name) {
        res.status(400).json({ error: 'schedule_id and file_name are required' });
        return;
      }
      const key = `sales/schedules/${schedule_id}/${Date.now()}_${sanitize(file_name)}`;
    ...
    if (req.method === 'GET' && path.endsWith('/signed-download-url')) {
      ...
      const signOpts = { version: 'v4', action: 'read', expires: Date.now() + SIGN_TTL_MS };
    ...
    if (req.method === 'GET') {
      ...
      if (!scheduleId) { res.status(400).json({ error: 'schedule_id is required' }); return; }
      const [rows] = await bigquery.query({
        query: `SELECT id, schedule_id, file_name, file_url, file_size, uploaded_by, uploaded_at
                FROM ${table} WHERE schedule_id = @schedule_id ORDER BY uploaded_at DESC`,
    ...
    if (req.method === 'POST') {
      const { schedule_id, file_name, file_url, file_size } = req.body;
      if (!schedule_id || !file_url) {
        res.status(400).json({ error: 'schedule_id and file_url are required' });
        return;
      }
      const id = uuidv4();
      await bigquery.query({
        query: `INSERT INTO ${table} (id, schedule_id, file_name, file_url, file_size, uploaded_by, uploaded_at)
                VALUES (@id, @schedule_id, @file_name, @file_url, @file_size, @uploaded_by, CURRENT_TIMESTAMP())`,
        params: { id, schedule_id, file_name: file_name || '', file_url, file_size: parseInt(file_size || 0), uploaded_by: email },
      });
    ...
    if (req.method === 'DELETE') {
      ...
      if (!id) { res.status(400).json({ error: 'id is required' }); return; }
      const [rows] = await bigquery.query({ query: `SELECT file_url FROM ${table} WHERE id = @id`, params: { id } });
      if (rows.length) {
        const parsed = parseKey(rows[0].file_url);
        if (parsed) {
