...
exports.invoice_files = async (req, res) => {
  setCors(res);
  ...
  try {
    // POST /invoice_files/signed-upload-url → { upload_url, file_url }
    if (req.method === 'POST' && path === '/signed-upload-url') {
      const { invoice_id, folder_id, file_name, content_type } = req.body || {};
      if (!invoice_id || !folder_id || !file_name) {
        res.status(400).json({ error: 'invoice_id, folder_id and file_name are required' });
        return;
      }
      const key = `${folder_id}/inv/${invoice_id}/${Date.now()}_${sanitize(file_name)}`;
    ...
    // GET /invoice_files/<id>/signed-download-url → { url }
    ...
    // GET /invoice_files?invoice_id=xxx
    if (req.method === 'GET') {
      const invoiceId = req.query.invoice_id;
      if (!invoiceId) { res.status(400).json({ error: 'invoice_id is required' }); return; }
      const [rows] = await bigquery.query({
        query: `SELECT id, invoice_id, name, file_name, file_url, file_size, uploaded_by, uploaded_at
                FROM ${table} WHERE invoice_id = @invoice_id ORDER BY uploaded_at DESC`,
    ...
    // POST — добавить файл (после успешной заливки через signed URL)
    if (req.method === 'POST') {
      const { invoice_id, name, file_name, file_url, file_size } = req.body;
      if (!invoice_id || !file_url) {
        res.status(400).json({ error: 'invoice_id and file_url are required' });
        return;
      }
      const id = uuidv4();
      await bigquery.query({
        query: `INSERT INTO ${table} (id, invoice_id, name, file_name, file_url, file_size, uploaded_by, uploaded_at)
                VALUES (@id, @invoice_id, @name, @file_name, @file_url, @file_size, @uploaded_by, CURRENT_TIMESTAMP())`,
        params: {
          id,
          invoice_id,
          name:        name      || file_name || '',
    ...
    // DELETE — удалить запись + физический файл из GCS
