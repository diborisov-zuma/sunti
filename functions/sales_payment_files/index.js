...
const DATASET  = 'sunti';
const TABLE    = 'sales_payment_files';
...
exports.sales_payment_files = async (req, res) => {
  setCors(res);
  ...
  try {
    // POST /sales_payment_files/signed-upload-url
    if (req.method === 'POST' && path === '/signed-upload-url') {
      const { payment_id, file_name, content_type } = req.body || {};
      if (!payment_id || !file_name) {
        res.status(400).json({ error: 'payment_id and file_name are required' });
        return;
      }
      const key = `sales/payments/${payment_id}/${Date.now()}_${sanitize(file_name)}`;
    ...
    if (req.method === 'GET') {
      ...
      if (!paymentId) { res.status(400).json({ error: 'payment_id is required' }); return; }
      const [rows] = await bigquery.query({
        query: `SELECT id, payment_id, file_name, file_url, file_size, uploaded_by, uploaded_at
                FROM ${table} WHERE payment_id = @payment_id ORDER BY uploaded_at DESC`,
    ...
    // POST — register file
    if (req.method === 'POST') {
      const { payment_id, file_name, file_url, file_size } = req.body;
      if (!payment_id || !file_url) {
        res.status(400).json({ error: 'payment_id and file_url are required' });
        return;
      }
      const id = uuidv4();
      await bigquery.query({
        query: `INSERT INTO ${table} (id, payment_id, file_name, file_url, file_size, uploaded_by, uploaded_at)
                VALUES (@id, @payment_id, @file_name, @file_url, @file_size, @uploaded_by, CURRENT_TIMESTAMP())`,
        params: {
          id, payment_id,
          file_name: file_name || '',
          file_url,
