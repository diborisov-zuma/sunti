...
const DATASET  = 'sunti';
const TABLE    = 'sales_contract_files';
...
exports.sales_contract_files = async (req, res) => {
  setCors(res);
  ...
  try {
    // POST /sales_contract_files/signed-upload-url
    if (req.method === 'POST' && path === '/signed-upload-url') {
      const { contract_id, sale_id, file_name, content_type } = req.body || {};
      if (!contract_id || !file_name) {
        res.status(400).json({ error: 'contract_id and file_name are required' });
        return;
      }
      const key = `sales/${sale_id || 'unknown'}/${contract_id}/${Date.now()}_${sanitize(file_name)}`;
